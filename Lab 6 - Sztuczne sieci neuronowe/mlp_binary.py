...
def vectorize(dataset_seq, vocabulary, classes):
    ...
    for label, msg in dataset_seq:
        ...
        target_value = torch.tensor([classes_stoi[label]], dtype=torch.float)
        dataset_bow.append({'input': feature_vector, 'target': target_value})
    return dataset_bow
...
classifier = Mlp(in_features=len(vocabulary))
...
for batch_index, batch_dict in enumerate(test_generator):
    ...
    # compute accuracy
    nb_correct = torch.eq(y_pred.round(), batch_dict['target']).sum().item()
    acc = nb_correct/len(batch_dict['target'])
    test_acc += (acc - test_acc) / (batch_index + 1)
